...
`flake.nix` (outputs) and `nix/package.nix` (the derivation) package stakk for nix.
One input, `nixpkgs` — no flake-utils, crane or rust-overlay,
because a single-crate build gains nothing from them and every input is another lock entry to keep current.

- **CI is the only verification.**
  The `nix` job in [`ci.yml`](.github/workflows/ci.yml) is the feedback loop: `nix flake check -L`, a build,
  and a run of the binary.
...
- **`cargoLock.lockFile`, not `cargoHash`.**
  Renovate runs lock file maintenance on `Cargo.lock`;
  a fixed-output hash would need re-pinning by hand on every one of those pull requests.
- **The `lib.fileset` source set is load-bearing.**
  `build.rs` reads `docs/`, and the unit tests read `src/**/snapshots`,
  `src/markdown/unwrap/test_corpus` and `docs/` back through `CARGO_MANIFEST_DIR` —
  a file left out of the set fails in the check phase, not at compile time.
- **`cargoTestFlags = [ "--bins" ]`** keeps `tests/e2e` out of the check phase: it needs a Forgejo container,
  which a nix build cannot reach.
  `.config/` is still in the source set
  so a check phase moved to nextest would inherit the same exclusion from `.config/nextest.toml` rather than reaching
  for an instance that is not there.
- **The check phase needs `SSL_CERT_FILE`.** reqwest resolves its TLS verifier when the client is built,
  and rustls-platform-verifier rejects an empty root store —
  so in a sandbox with no system trust store the two `forge::forgejo::transport` tests
  that build a client fail with `No CA certificates were loaded from the system`.
  `preCheck` points `SSL_CERT_FILE` at `cacert`, which rustls-native-certs reads.
  Skipping those two tests instead would drop the coverage of `install_crypto_provider`,
  which is the whole reason they exist.
- Shell completions are generated by running the binary that was just built; `stakk completions` needs neither jj
  nor a repository.
- The package does **not** wrap the binary with jj on `PATH`.
  jj is a runtime requirement the user supplies, and stakk's own version check reports a missing or too-old one.
...
flake.nix            # Nix flake outputs (package, overlay, checks, devShell)
nix/package.nix      # The buildRustPackage derivation — see "Nix packaging"
src/
