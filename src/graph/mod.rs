...
mod tests {
    ...
    fn bookmark_json(name: &str, commit_id: &str, change_id: &str) -> String {
        format!(
            r#"{{"name":"{name}","target":{{"commit_id":"{commit_id}","parents":[],"change_id":"{change_id}","description":"","author":{{"name":"T","email":"t@t.t","timestamp":"T"}},"committer":{{"name":"T","email":"t@t.t","timestamp":"T"}}}}}}"#,
        )
    }
