...
pub fn resolve_bookmark_interactively(graph: &ChangeGraph) -> Result<Option<String>, StakkError> {
    ...
    if total_bookmarks == 1 {
        ...
        let summary = segment
            ...
            .filter(|l| !l.is_empty())
            .unwrap_or("(no description)");
        let confirmed = inquire::Confirm::new(&format!("Submit {name}? ({summary})",))
            .with_default(true)
            .prompt()
            .map_err(map_inquire_error)?;
        if confirmed {
...
mod tests {
    ...
    // Note: the single-bookmark case now shows a confirmation prompt
    // (inquire::Confirm), which cannot be driven from a unit test
    // without a TTY. Interactive paths are tested manually via the
    // jack-testing repo.
}
